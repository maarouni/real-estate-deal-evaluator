...
def generate_pdf(property_data, metrics_a, metrics_b, summary_text):
    buffer = BytesIO()
...
def generate_comparison_pdf(metrics_a, metrics_b):
    buffer = BytesIO()
    c = canvas.Canvas(buffer, pagesize=letter)
    c.drawString(100, 750, "Comparison Report")

    # Example summary values
    c.drawString(100, 730, f"Cap Rate A: {metrics_a.get('Cap Rate (%)', 'N/A')}")
    c.drawString(100, 710, f"Cap Rate B: {metrics_b.get('Cap Rate (%)', 'N/A')}")

    c.drawString(100, 690, f"Final Year ROI A: {metrics_a.get('Final Year ROI (%)', 'N/A')}")
    c.drawString(100, 670, f"Final Year ROI B: {metrics_b.get('Final Year ROI (%)', 'N/A')}")

    c.drawString(100, 650, f"Cash-on-Cash Return A: {metrics_a.get('Cash-on-Cash Return (%)', 'N/A')}")
    c.drawString(100, 630, f"Cash-on-Cash Return B: {metrics_b.get('Cash-on-Cash Return (%)', 'N/A')}")
...
def generate_comparison_pdf_table_style(metrics_a, metrics_b):
    buffer = BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=letter)
    elements = []
