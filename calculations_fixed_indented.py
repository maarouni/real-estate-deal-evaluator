    ...
    def calculate_metrics(purchase_price, monthly_rent, down_payment_pct, interest_rate, loan_term,
        ...
        # --- Prepare investment base values ---
        #initial_investment = purchase_price * (down_payment_pct / 100)

        # --- Build multi-year cash flow list ---
        cash_flows = []
        current_rent = annual_rent

        for year in range(1, time_horizon + 1):
        # Apply appreciation and rent growth over years
            current_rent *= (1 + rent_growth_rate / 100)
            property_value = purchase_price * ((1 + appreciation_rate / 100) ** year)

        # Compute annual cash flow for this year
            annual_cash_flow = current_rent - annual_expenses - annual_mortgage
            cash_flows.append(round(annual_cash_flow, 2))

    # Fallback IRR function using NPV and Newton's method
    ...
    print("🧪 IRR cash flows (for debug):", irr_cash_flows)
    irr = robust_irr(irr_cash_flows)

    # Continue with ROI or other metrics as needed...


        # --- Now compute IRR and Equity Multiple ---
       #initial_investment = purchase_price * down_payment_pct / 100
        #irr_cash_flows = [-initial_investment] + cash_flows
        #print("🧮 IRR Debug – Cash Flows Input:", irr_cash_flows)

        #try:
            #irr_raw = npf.irr(irr_cash_flows)
            #irr = round(irr_raw * 100, 2) if irr_raw is not None and not np.isnan(irr_raw) else 0
        #except:
            #irr = 0
    ...
    def robust_irr(cash_flows, guess=0.1):
        ...
        return {
            ...
            "Multi-Year Cash Flow": [round(x, 2) for x in annual_cash_flows],       # ✅ NEW: Rounded
            "Annual ROI % (by year)": [round(x, 2) for x in roi_list],              # ✅ NEW: Rounded
            "Annual Rents $ (by year)": [round(x, 2) for x in rents],               # ✅ NEW: Rounded
            "irr (%)": irr,                           # ✅ IRR added here
            "equity_multiple": equity_multiple        # ✅ Equity Multiple added here
        }