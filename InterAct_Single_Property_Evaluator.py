...
# ✅ Safely extract from calculated_metrics dictionary
multi_year_cash_flow = metrics.get("Multi-Year Cash Flow", [])
annual_rents = metrics.get("Annual Rents $ (by year)", [])
roi_per_year = metrics.get("Annual ROI % (by year)", [])
# 🔢 Generate x-axis labels based on number of years
years = list(range(1, len(multi_year_cash_flow) + 1))

# 📈 Plotly Interactive Chart
fig = go.Figure()
fig.add_trace(go.Scatter(x=years, y=multi_year_cash_flow, mode="lines+markers", name="Cash Flow"))
fig.add_trace(go.Scatter(x=years, y=annual_rents, mode="lines+markers", name="Projected Rent"))
fig.add_trace(go.Scatter(x=years, y=roi_per_year, mode="lines+markers", name="ROI (%)", yaxis="y2"))
...
fig.update_layout(
    ...
    xaxis_title="Year",
    yaxis=dict(title="Cash Flow / Rent ($)", side="left"),
...
# 📈 Interactive Multi-Year Cash Flow Projection
st.subheader("📈 Multi-Year Cash Flow & ROI (Interactive)")

# Prepare x-axis
years = list(range(1, time_horizon + 1))

# User toggles
show_cashflow = st.checkbox("Show: Multi-Year Cash Flow", value=True)
show_rent = st.checkbox("Show: Projected Rent", value=True)
...
if show_cashflow:
    fig.add_trace(go.Scatter(
        x=years, y=metrics["Multi-Year Cash Flow"],
        mode='lines+markers', name="Cash Flow ($)"
    ))

if show_rent:
    fig.add_trace(go.Scatter(
        x=years, y=metrics["Annual Rents $ (by year)"],
        mode='lines+markers', name="Projected Rent ($)"
    ))

if show_roi:
    fig.add_trace(go.Scatter(
        x=years, y=metrics["Annual ROI % (by year)"],
        mode='lines+markers', name="ROI (%)", yaxis="y2"
    ))

# Layout with dual y-axis
fig.update_layout(
    title="Multi-Year Cash Flow vs Rent vs ROI",
    xaxis_title="Year",
    yaxis=dict(title="Cash Flow / Rent ($)"),
